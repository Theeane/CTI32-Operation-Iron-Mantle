/* PRESET: Arma 3 Vanilla Civilians
    Author: Theane using Gemini
    Framework: CTI32 - Operation Iron Mantle
*/

// --- Civila enheter (Blandning av casual och arbetare) ---
GVAR_CIV_Units = [
    "C_man_1", "C_man_polo_1_F", "C_man_polo_2_F", 
    "C_man_p_beggar_F", "C_man_1_1_F", "C_man_hunter_1_F",
    "C_man_shorts_1_F", "C_idap_man_shorts_01_F", "C_idap_man_casual_01_F"
];

// --- Civila fordon (Standard bilar och skåpbilar) ---
GVAR_CIV_Vehicles = [
    "C_Offroad_01_F",                   // Offroad
    "C_Offroad_02_unarmed_F",           // MB 4WD (Apex)
    "C_Hatchback_01_F",                 // Hatchback
    "C_Hatchback_01_sport_F",           // Hatchback (Sport)
    "C_SUV_01_F",                       // SUV
    "C_Van_01_transport_F",             // Truck Boxer
    "C_Van_02_vehicle_F",               // Van (Orange DLC)
    "C_Truck_02_transport_F"            // Zamak Transport
];
