...
diag_log format ["[MWF] Campaign state restored. Pending saved zones: %1 | Pending vehicles: %2 | Pending active missions: %3 | Auth Players: %4 | Analytics Rows: %5 | Existing campaign save: %6.", count (missionNamespace getVariable ["MWF_LoadedZoneSaveData", []]), count (missionNamespace getVariable ["MWF_PendingBoughtVehicles", []]), count (missionNamespace getVariable ["MWF_PendingActiveSideMissions", []]), count (missionNamespace getVariable ["MWF_AuthenticatedPlayers", []]), count (missionNamespace getVariable ["MWF_Campaign_Analytics", []]), _hasCampaignSave];
