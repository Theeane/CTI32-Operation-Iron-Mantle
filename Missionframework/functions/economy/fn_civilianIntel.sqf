/*
    Author: Theeane
    Function: CTI_fnc_civilianIntel
    Description: 
    Handles interaction with civilians to gain intel points.
    Includes a cooldown and scaling probability.
*/
...
if (_civ getVariable ["GVAR_isQuestioned", false]) exitWith {
...
if ((random 100) <= _findChance) then {
    ...
    private _intelGained = GVAR_Economy_Intel_CivilianTalk;
    ...
    _civ setVariable ["GVAR_isQuestioned", true, true];
} else {
