...
// 1. Define the composition offsets (Same as MWF_fn_spawnFOBComposition)
...
while {!_confirmed && !_abort && alive player} do {
    ...
    private _centerPos = player modelToWorld [0, 15, 0];
...
if (_confirmed) then {
    private _finalCenter = player modelToWorld [0, 15, 0];
    ...
    diag_log format ["[MWF FOB] FOB placement confirmed at %1 with direction %2.", _finalCenter, _currentDir];
} else {
